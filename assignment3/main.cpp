...
T DoublyLinkedList<T>::get(const unsigned int index) const {
    ...
    while(currentNodeNum < index){
        currentNode = currentNode->forward;  //iterate through the linked list
...
void DoublyLinkedList<T>::insert(const unsigned int index, const T &value){
        ...
        //if there's only one node, or if its inserting into the last index
        if(this->front == this->back || (index-1)-this->count == 1){
            ++currentNodeNum;
        }
        ...
        //if there is only 1 node
            ...
            this->front->forward = temp;
            temp->backward = this->front;
            this->back = temp;
        }
        //if its inserting before first node
        else if(this->front == currentNode){
...
void DoublyLinkedList<T>::remove(const unsigned int index){
    ...
    //if there's only one node, or if its deleting the last index
    ...
    //scenario: if there are no nodes
    if(!this->front){
        cout << "This list is empty" << endl;
        return;
    }
    //specific scenario; one node list
    else if(this->front == this->back){
        delete this->front;
        this->front = nullptr;
        this->back = nullptr;
    ...
    //if its the front
    else if(currentNode == this->front){
    ...
    //if its the middle
        ...
        Node<T> * temp{currentNode->forward}; //temp is intialized to back
        temp->backward = currentNode->backward;
        currentNode->backward->forward = temp;
        delete currentNode;
        this->count--;
...
//remove all instances function
template <typename T>
void DoublyLinkedList<T>::removeAllInstances(const T &value){
    Node<T> * temp{this->front}; //temp is intialized to back
    Node<T> * prev{this->front}; //prev is also initialized to front
    ...
    while(temp != nullptr){
        if(temp->data == value){
           ...
           //if its the last node
               ...
               //if its a middle node
               prev->forward = temp->forward;
               temp->forward->backward = prev;
               delete temp;
               temp = prev->forward;
               this->count--;
           }
